...
const SocialSharePopup = ({ url, title, description = "", image, children }: SocialSharePopupProps) => {
  ...
  // Build share text with signature
  const shareText = `${title}\n\n${description}\n\n— Inocent KOFFI | Fondateur & CEO AGRICAPITAL SARL`;
  ...
  const shareLinks = [
    ...
    },
    {
      ...
      url: `https://wa.me/?text=${encodedShareText}%0A%0A${encodedUrl}`,
    ...
    },
    {
      ...
      url: `mailto:?subject=${encodedTitle}&body=${encodeURIComponent(shareText)}%0A%0A${encodedUrl}`,
    ...
    },
    {
      ...
      url: `sms:?body=${encodeURIComponent(shareText)}%0A${encodedUrl}`,
  ...
  const handleShare = async (link: string, platformName: string) => {
    ...
    if (postIdMatch) {
      // Look up post_id by slug
      const { data: postData } = await supabase
        .from('blog_posts')
        .select('id')
        .eq('slug', postIdMatch[1])
        .single();
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="grid grid-cols-4 gap-3 py-4">
          {shareLinks.map((link) => (
            ...
            >
              {link.icon}
              <span className="text-xs mt-1.5 font-medium">{link.name}</span>
            </button>
          ))}
        </div>
