...
const SocialSharePopup = ({ url, title, description = "", image, children }: SocialSharePopupProps) => {
  const [copied, setCopied] = useState(false);
  const [open, setOpen] = useState(false);
  const { toast } = useToast();

  // Build the OG-enabled share URL that serves proper meta tags to crawlers
  const slug = url.match(/\/blog\/(.+)$/)?.[1];
  const ogShareUrl = slug
    ? `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/og-article?slug=${encodeURIComponent(slug)}`
    : url;

  // Professional share text: summary + signature, NO URL in the text
  ...
  const shareLinks = [
    {
      ...
      url: `https://www.facebook.com/sharer/sharer.php?u=${encodedOgUrl}`,
    ...
    },
    {
      ...
      url: `https://wa.me/?text=${encodedShareBody}%0A%0A${encodedOgUrl}`,
    ...
    },
    {
      ...
      url: `https://www.facebook.com/dialog/send?link=${encodedOgUrl}&app_id=291494419107518&redirect_uri=${encodeURIComponent(url)}`,
      color: "bg-[#0099FF] hover:bg-[#008AE6]",
    },
    {
      name: "LinkedIn",
      icon: (
      ...
      url: `https://www.linkedin.com/sharing/share-offsite/?url=${encodedOgUrl}`,
      color: "bg-[#0A66C2] hover:bg-[#095BA5]",
    },
    {
      name: "X (Twitter)",
      icon: (
      ...
      url: `https://twitter.com/intent/tweet?text=${encodedShareBody}&url=${encodedOgUrl}`,
    ...
    },
    {
      ...
      url: `mailto:?subject=${encodedTitle}&body=${encodedShareBody}`,
    ...
    },
    {
      ...
      url: `sms:?body=${encodedShareBody}`,
      color: "bg-[#5BB974] hover:bg-[#4EA965]",
    },
  ];
  ...
  const handleShare = async (link: string, platformName: string) => {
    // Track share
        ...
        <div className="grid grid-cols-4 gap-3 py-3">
          {shareLinks.map((link) => (
            <button
              ...
              className={`flex flex-col items-center justify-center p-3 rounded-xl text-white transition-all hover:scale-105 active:scale-95 ${link.color}`}
